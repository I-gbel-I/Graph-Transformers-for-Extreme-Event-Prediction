...
def train_val_pipeline(model_name, dataset, params, net_params, dirs):
    ...
    options = pd.DataFrame({'ID': [0, 1, 2], 'Loss': ['Mean Squared Error (MSE)', 'Squared Error-Relevance Area (SERA)', 'Weighted Focal MSE']})
    ...
    try:
        ...
        with tqdm(range(params['epochs'])) as t:
            ...
            for epoch in t:
                ...
                epoch_val_loss = evaluate_network(model, device, val_loader, selected_loss)
    ...
    train_mse_bulk, train_mse_extremes = MSE_comparison.by_target_type(train_loader, 'training set')
    test_mse_bulk, test_mse_extremes = MSE_comparison.by_target_type(test_loader, 'testing set')

    print(f'\nTrain MSE (bulk & extremes): {train_mse_bulk} & {train_mse_extremes}')
    print(f'Test MSE (bulk & extremes): {test_mse_bulk} & {test_mse_extremes}')
